...
function ImageEditor::reCreateImage(%this)
{
   ...
   %imageOptimised = ImageEditorCheckBoxOptimised.getValue();
   ...
   switch$ (%imageMode)
   {
      case "FULL":
         ...
         %imageMap.compressPVR = %imageCompressPVR;
         %imageMap.optimised = %imageOptimised;
...
{  
   ...
   %compressPVR = %imageMap.compressPVR;
   %optimised = %imageMap.optimised;
   ...
   ImageEditorCheckBoxOptimised.setValue(%optimised);
   
   // CHRIS:   Originally you could set the check boxes for these values and save
   ...
   $lastComoressPVRValue = %compressPVR;
   $lastPreloadValue = %preload;
   $lastAllowUnloadValue = %allowUnload;
   $lastPreferPerfValue = %preferPerf;
   $lastFilterPadValue = %filterPad;
   $lastOptimisedValue = %optimised;
...
function ImageEditorOptimised()
{
   %value = ImageEditorOptimised.getValue();
   
   if($lastOptimisedValue $= "" || %value != $lastOptimisedValue)
   {
      $lastOptimisedValue = %value;
      ImageEditorAutoApply();
   }
}
