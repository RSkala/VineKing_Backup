...
$T2D::ImageMapSpec = "All Supported Graphics (*.jpg;*.bmp;*.png)|*.png;*.jpg;*.bmp|";
...
function copyDB(%oldName, %newName)
{
   ...
   if(!%oldName.compressPVR)
      %oldName.compressPVR = 0;
   ...
   %datablock = %datablock @ "{\n";
      ...
      %datablock = %datablock @ "compressPVR = " @ %oldName.compressPVR @ ";\n";
      %datablock = %datablock @ "optimised = " @ %oldName.optimised @ ";\n";
...
function restoreDB(%oldName, %newName, %name)
{
   ...
   %newName.compressPVR = %oldName.compressPVR;
   %newName.optimised = %oldName.optimised;
