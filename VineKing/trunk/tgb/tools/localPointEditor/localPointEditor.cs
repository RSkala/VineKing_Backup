...
/// terms of the -1 to 1 "local space" of an object, such as linkpoints or collision
/// polygon points.
///
/// Modes!
...
/// displayScenegraph - The scenegraph that holds all of the sceneobjects for the editor
/// editObject - The representation of the object we're editing inside the editor
/// displayPolygon - A t2dShapeVector that shows a polygon connecting the local points
/// hullPoly - A t2dShapeVector that shows a polygon of the convex hull of the local points
...
{
   // clean up the scenegraph object, since we're tearing everything down.
   if (isObject(%this.displayScenegraph))
   {
      %this.displayScenegraph.delete();
   }
...
function LocalPointEditor::open(%this, %object)
{
   ...
   LEVisualEdit.setSceneGraph(%this.displayScenegraph);
   if (isObject(%object))
   {
...
function LocalPointEditor::editLocalPoints(%this, %object)
{
   ...
   %this.editObject = %object.cloneWithBehaviors();
   %this.editObject.class = "";
   %this.editObject.superclass = "";
   %this.editObject.clearBehaviors();
   %this.editObject.setWorldLimit(OFF);
   %this.displayScenegraph.add(%this.editObject);
   ...
   %this.editObject.setLayer($LocalPointEditor::editObjectLayer);
   %this.editObject.setGraphGroup($LocalPointEditor::ignoreGroup);
   %this.editObject.setPosition(0 SPC 0);
   //%this.editObject.setSize(t2dVectorScale(%this.aspect, 2));
   %this.updateAspect();
   %this.editObject.setVisible(%this.showEditObject);
   %this.displayPolygon.setSize(t2dVectorScale(%this.aspect, 2));
   %this.hullPoly.setSize(t2dVectorScale(%this.aspect, 2));
...
function LocalPointEditor::createNewLocalPoint(%this, %position)
{
   ...
   %lp.dot.setSize(t2dVectorScale($LocalPointEditor::dotSize, %zoom));
   ...
   LEVisualEdit.getSceneGraph().add(%lp.dot);   
   LEVisualEdit.getSceneGraph().add(%lp.label);
...
function LocalPointEditor::updateGridLines(%this)
{
   ...
   %halfSize = t2dVectorScale(%size, 0.5);
   %halfSize = %this.sceneSpaceToLocalSpace(%halfSize);

   
   %min = t2dVectorSub(%pos, %halfSize);
   %max = t2dVectorAdd(%pos, %halfSize);
   ...
   for (%x=%gridSpacing*mFloor(%minX/%gridSpacing); %x<%maxX; %x+= %gridSpacing)
   {
      ...
      if (%i >= %this.vertLines.getCount())
      {
         ...
         LEVisualEdit.getSceneGraph().add(%line);
      }
      else
      {
...
function LocalPointEditor::updateAspect(%this)
{
   ...
   if (%this.editObject.getClassName() $= "t2dTileLayer")
   {
      ...
      %this.editObject.setTileSize(t2dVectorMult(%this.baseObject.getTileSize(), %tileScale));
   }
   else if (%this.editObject.getClassName() $= "t2dScroller")
   {
      %mySize = %this.editObject.getSize();
      %baseSize = %this.baseObject.getSize();
      %scrollerScale = getWord(%mySize, 0) / getWord(%baseSize, 0) SPC getWord(%mySize, 1) / getWord(%baseSize, 1);
      %this.editObject.setScroll(t2dVectorMult(%this.baseObject.getScrollX() SPC %this.baseObject.getScrollY(), %scrollerScale));
   }
...
function LocalPointEditor::setZoom(%this, %zoom)
{
   ...
   %dotSize = t2dVectorScale($LocalPointEditor::dotSize, %zoom);
...
function LocalPointEditor::updateBackgroundCam(%this)
{
   ...
   if (%updateBG)
   {
      LEBackgroundDisplay.setDontRenderObject(%this.baseObject);
      LEBackgroundDisplay.setSceneGraph(%this.baseObject.getSceneGraph());
   
      %size = t2dVectorScale(%this.baseObject.getSize(), LPZoomSlider.value);
...
{
   // return the local space coordinates, based on the scenegraph we're using to display.
...
{
   // return the scenegraph coordinates in the display, based on local space coordinates.
...
function LocalPointEditor::findClosestSegment(%this, %pos)
{
   ...
   for (%i=0; %i<%numVertices; %i++)
   {
      ...
      %segLength = t2dVectorDistance(%curPos, %nextPos);
      ...
      if ((%u>0) && (%u<1))
      {
         ...
         %length = t2dVectorDistance(%pos, %px SPC %py);   
      }
      else
      {
         %l1 = t2dVectorDistance(%curPos, %pos);
         %l2 = t2dVectorDistance(%nextPos, %pos);
