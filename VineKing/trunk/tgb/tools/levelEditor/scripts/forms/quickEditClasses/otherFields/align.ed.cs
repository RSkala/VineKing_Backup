...
function LBQuickEditContent::createAlignTools(%this, %singleObject)
{
   ...
   for( %dir = 0; %dir <= 1; %dir++ )
   {
      for( %side = -1; %side <= 1; %side++ )
      {
         ...
         %button = new GuiIconButtonCtrl()
         {
            ...
            side = %side;
            dir = %dir;
            iconBitmap = "~/levelEditor/gui/images/iconAlign" @ %text;
         };
      ...
      for( %dir = 0; %dir <= 1; %dir++ )
      {
         for( %side = -1; %side <= 1; %side++ )
         {
            ...
            %button = new GuiIconButtonCtrl()
            {
               ...
               side = %side;
               dir = %dir;
               iconBitmap = "~/levelEditor/gui/images/iconDistribute" @ %text;
            };
   ...
   for( %dir = 0; %dir <= 2; %dir++ )
   {
      ...
      %button = new GuiIconButtonCtrl()
      {
         ...
         dir = %dir;
         iconBitmap = "~/levelEditor/gui/images/iconMatchSize" @ %text;
      };
   ...
   if( !%singleObject )
   {
      ...
      for( %dir = 0; %dir <= 1; %dir++ )
      {
         ...
         %button = new GuiIconButtonCtrl()
         {
            ...
            class = "SpaceButton";
            dir = %dir;
            iconBitmap = "~/levelEditor/gui/images/iconSpace" @ %text;
         };
...
function AlignButton::onClick( %this )
{
   ...
   if( %toCamera )
      AlignTools::alignToCamera( %objects, ToolManager.getLastWindow().getSceneGraph(), %this.side, %this.dir );
   else
...
function DistributeButton::onClick( %this )
{
   ...
   if( %toCamera )
      AlignTools::distributeToCamera( %objects, ToolManager.getLastWindow().getSceneGraph(), %this.side, %this.dir );
   else
...
function MatchSizeButton::onClick( %this )
{
   ...
   if( %toCamera )
      AlignTools::matchSizeToCamera( %objects, ToolManager.getLastWindow().getSceneGraph(), %this.dir );
   else
...
function SpaceButton::onClick( %this )
{
   ...
   if( %toCamera )
      AlignTools::spaceToCamera( %objects, ToolManager.getLastWindow().getSceneGraph(), %this.dir );
   else
