...
#include "T2D/t2dVector.h" // RKS: I ADDED THIS
#endif
...
{
   addProtectedField( "Enabled", TypeBool, Offset(mEnabled, SimComponent), &setEnabled, &defaultProtectedGetFn, "" );
...
{
   //Con::executef(this, 2, "onComponentAdd", Con::getIntArg(target->getId())); // RKS: I disabled this script call
   return true;
}

void SimComponent::onComponentRemove(SimComponent *target)
{
   //Con::executef(this, 2, "onComponentRemove", Con::getIntArg(target->getId())); // RKS: I disabled this script call
}
...
#ifdef USE_COMPONENTS
//-Mat shouldn't ever really do anything
void SimComponent::onUpdate() {
}
void SimComponent::onAddToScene() {
}


//----------------------------------------------------------------------------------------------------------------------------------------------------------------

void SimComponent::OnMouseEvent( const EMouseEvent& _eMouseEvent, const t2dVector& _vWorldMousePoint ) // RKS: I added this
{
	switch( _eMouseEvent )
	{
		case MOUSE_EVENT_DOWN:
			OnMouseDown( _vWorldMousePoint );
			break;
		...
		case MOUSE_EVENT_ENTER:
			OnMouseEnter( _vWorldMousePoint );
			break;
			
		case MOUSE_EVENT_LEAVE:
			OnMouseLeave( _vWorldMousePoint );
			break;
			
		default:
			//printf( "SimComponent::%s - Unhandled mouse event: %d\n", __FUNCTION__, _eMouseEvent );
			break;
	}
}
...
#ifdef USE_COMPONENTS
ConsoleFunction( copyBehaviorToComponent, int, 3, 3, "(behavior, component)Copies a behaviors values to a component" ) {
	int bID = dAtoi( argv[1] );
	int sID = dAtoi( argv[2] );
	BehaviorInstance *pBehavior = (BehaviorInstance *)Sim::findObject( bID );
	BehaviorTemplate *pTemplate = pBehavior->getTemplate();

	SimComponent *pComponent = (SimComponent *)Sim::findObject( sID );
	if( !pComponent ) {
		Con::errorf( "copyBehaviorToComponent cannot find component with ID %i", sID );
		return 0;
	}

	//the template will only give us teh deafult values, what if we want the instance specific ones?
	int fieldCount = pTemplate->getBehaviorFieldCount();
	
	/*
		Here we get the name of the value from the template, but we want to get the actual value from
		the instance. For some reason, you can't loop through the values in the instance by an ID.
		-Mat
	*/
	const char *fieldValue = NULL;
	BehaviorTemplate::BehaviorField *bField = NULL;
	for( int i = 0; i < fieldCount; i++ ) {
		bField = pTemplate->getBehaviorField( i );
		fieldValue = pBehavior->getDataField( bField->mFieldName, NULL );
		//we now have the value, and the name of the variable, though we get them from two different places

		if( fieldValue != NULL ) {
			//this seems to have trouble setting pointers?
			pComponent->setDataField( bField->mFieldName, NULL, fieldValue );
		} else {
			break;
			return false;
		}
