...
#include "T2D/t2dPhysics.h" // RKS: Did I add this?
#endif

#include "MouseEventEnums.h" // RKS

// Forward refs
class Stream;
class t2dVector; // RKS: Forward Declaration for the new SimComponent::OnMouseEvent
...
//%PUAP% -Mat adding onUpdate to components
   virtual void onUpdate();
   virtual void onAddToScene();   
	
	// RKS: I added these, so components can respond to mouse events. Also, I currently don't use the GuiEvent object at all, so I do not pass it to these functions.
	
	virtual void OnMouseEvent( const EMouseEvent& _eMouseEvent, const t2dVector& _vWorldMousePoint );
	...
	virtual void HandleOwnerCollision( t2dPhysics::cCollisionStatus* _pCollisionStatus ) {}
