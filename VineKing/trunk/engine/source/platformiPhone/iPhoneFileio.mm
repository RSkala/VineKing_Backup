...
void iPhoneSaveStringToDevice( const char* saveNameField, const char* dataString)
{
...
static const char* iPhoneLoadStringFromDevice(const char* saveNameField)
{
	NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
	NSString *convertedSaveFieldName = [[NSString alloc] initWithUTF8String:saveNameField];
...
ConsoleFunction(iPhoneSaveStringToDevice, void, 3, 3, "(keyName, keyString) Save's a string value to a key defined in the iPhone user defaults. This can be seen as similar to the windows registry.\n"
												   "@keyString: The string to store in this key.\n"
												   "@keyName: The name for the key to store the string under.")
{
	iPhoneSaveStringToDevice( argv[1] , argv[2] );
}

ConsoleFunction(iPhoneLoadStringFromDevice, const char*, 2, 2, "(keyName) Loads a string from a key defined in the iPhone user defaults (saved with iPhoneSaveStringToDevice)\n"
														   "@keyName: The name for the string that it was stored under.")
{
	const char* result = iPhoneLoadStringFromDevice( argv[1] );
	return result;
}
