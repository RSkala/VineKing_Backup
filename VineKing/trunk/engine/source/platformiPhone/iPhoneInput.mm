...
#define IPHONE_DOUBLE_CLICK_TIME	( 50 * 60.0f * 1000.0f)
...
void Input::init()
{
   ...
   smManager = NULL;
	//smManager = new iPhoneInputManager();
   smActive = false;
...
//------------------------------------------------------------------------------
ConsoleFunction( isJoystickDetected, bool, 1, 1, "Always false on the iPhone." )
{
/*
   argc; argv;
...
{
	return NULL;//no clipboard on iPhone
}
...
{
	return IPHONE_DOUBLE_CLICK_TIME;
}
		...
		the script should look something like this:
			function oniPhoneTouchDown( %touchCount, %touchX, %touchY ) {
				for( %i = 0; %i < %touchCount; %i++ ) {
					echo( "Touch #" @ %i @ "x:" SPC getWord( %touchY, %i ) SPC "y:" SPC getWord( %touchY, %i ) );
				}
			}
	*/
	
	char posX[256], posY[256], temp[10];
	posX[0] = '\0';
	posY[0] = '\0';
	...
	if( numTouchDownEvents > 0 ) {
		Con::executef( 4, "oniPhoneTouchDown", temp , posX, posY );
	}
	...
	if( numTouchMoveEvents > 0 ) {
		Con::executef( 4, "oniPhoneTouchMove", temp , posX, posY );
	}
	...
	if( numTouchUpEvents > 0 ) {
		Con::executef( 4, "oniPhoneTouchUp", temp , posX, posY );
	}
	...
	if( numTapEvents > 0 ) {
		Con::executef( 4, "oniPhoneTouchTap", temp , posX, posY );
		//printf("oniPhoneTouchTap(%s,%s,%s)\n", temp , posX, posY);
	}
...
bool createMouseUpEvent( S32 touchNumber, S32 x, S32 y ) 
{
	...
	//printf( "createMouseUpEvent - Number: %d; Position: (%d, %d)\n", touchNumber, x, y ); // RKS: Testing for touch up bullshit
