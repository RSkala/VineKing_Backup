...
#include "platformiPhone/iPhoneTextEntry.h"
#include "platformiPhone/platformiPhone.h"
#include "platformiPhone/iPhoneTextEntryController.h"

extern void _iPhoneGameInnerLoop();

bool iPhoneTextEntry::onUserTextFinished(bool cancelled, const char* &result)
{
	Con::executef(3,"oniPhoneKeyboardInputFinished",Con::getIntArg(0), Con::getIntArg(cancelled), Con::getReturnBuffer(result));
	Con::setVariable("$iPhone::keyboardWasCancelled", Con::getIntArg(cancelled));
...
bool iPhoneTextEntry::getUserText(StringBuffer& text)
{
	...
	iPhoneTextEntryController* controller = [[iPhoneTextEntryController alloc] initWithNibName: @"iPhoneTextEntry" bundle: nil];
...
//	controller.view.transform = CGAffineTransformMakeTranslation(0, 0);
	...
	[controller release];
	Con::setVariable("$iPhone::keyboardWasCancelled", Con::getIntArg(userCanceled));
	...
	*/
	...
	/*
	iPhoneTextEntryController* controller = [[iPhoneTextEntryController alloc] initWithNibName: @"TextEntry" bundle: nil];
	UIWindow* window = [UIApplication sharedApplication].keyWindow;
   [controller loadView];
