...
int giPhoneGameCurrentOrientation = iPhoneOrientationLandscapeLeft;
...
//Luma:	Orientation support
int _iPhoneGameGetOrientation()
{
	return giPhoneGameCurrentOrientation;
}
void _iPhoneGameSetCurrentOrientation(int iOrientation)
{
	giPhoneGameCurrentOrientation = iOrientation;
}
...
ConsoleFunction(OpeniPhoneRadio, void, 2, 2, "Forces open the iPhone radio if given a Torque formatted IP:port combo (255.255.255.255:65535)\n\n")
{
	if(IsDeviceiPhone())
	{
		...
		if(!gpTCPObject)
		{
			OpeniPhoneNetworkingAndConnectToTCPObject(NULL, argv[1]);
		}
	}
}