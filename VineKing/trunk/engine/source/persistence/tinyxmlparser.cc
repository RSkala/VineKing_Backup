...
TiXmlNode* TiXmlNode::Identify( const char* p )
{
   ...
   else if ( StringEqual( p, dataHeader, false ) )
   {
		#ifdef DEBUG_PARSER
			TIXML_LOG( "XML parsing Data\n" );
		#endif
		returnNode = new TiXmlData( "" );
   }
	else
	{
		#ifdef DEBUG_PARSER
...
#ifdef TIXML_USE_STL
void TiXmlData::StreamIn( TIXML_ISTREAM * in, TIXML_STRING * tag )
{
   while ( in->good() )
	{
		int c = in->get();	
...
const char* TiXmlData::Parse( const char* p )
{
	TiXmlDocument* document = GetDocument();
	value = "";
	...
	if ( !StringEqual( p, startTag, false ) )
	{
		document->SetError( TIXML_ERROR_PARSING_DATA );
		return 0;
	}
	p += strlen( startTag );
	p = ReadText( p, &value, false, endTag, false );
