...
S32 SimXMLDocument::loadFile(const char* rFileName)
{
   ...
   char temp[1024];
   Con::expandScriptFilename(temp, sizeof(temp), rFileName);
...
S32 SimXMLDocument::saveFile(const char* rFileName)
{
...
#else
   char buffer[1024];
   Con::expandToolScriptFilename(buffer, sizeof(buffer), rFileName);
#endif
   ...
   bool retVal = m_qDocument->SaveStream(strm);//m_qDocument->SaveFile( buffer );
...
const char* SimXMLDocument::getData()
{
   ...
   TiXmlData* text = pNode->FirstChild()->ToData();
   if( !text )
      return "";
