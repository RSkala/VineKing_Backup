// NOTICE: THIS FILE HAS BEEN MODIFIED FROM IT'S ORIGNAL FORM
//         MODIFICATIONS HAVE BEEN MADE TO MAKE IT FIT THE TORQUE SDK.
...
// UNUSED: JOSEPH THOMAS -> static char* streamBuf[1024];
...
void TiXmlElement::Print( Stream& stream, int depth ) const
{
	int i;
	for ( i=0; i<depth; i++ )
	{
	...
	stream.write(1, "<" );
   stream.write( value.length(), value.c_str() );
...
bool TiXmlDocument::LoadFile( const char* fileName )
{
   ...
   //ResourceObject *obj = ResourceManager->find (fileName);
   //if (!obj)
     // return false;
   ...
   //value = buildPath(obj->path, obj->name);
   if(diskStream->open ( value.c_str(), FileStream::Read))
   {

      bool check = LoadStream( *diskStream );
...
bool TiXmlDocument::SaveStream( FileStream& stream )
{
   Print( stream, 0 );
   return true;
}
...
void TiXmlDocument::Print( Stream& stream, int depth ) const
{
	TiXmlNode* node;
	for ( node=FirstChild(); node; node=node->NextSibling() )
	{
...
void TiXmlAttribute::Print( Stream& stream, int /*depth*/ ) const
{
	TIXML_STRING n, v;
   ...
   {
		//fprintf (cfile, "%s=\"%s\"", n.c_str(), v.c_str() );
      stream.write(n.length(), n.c_str() );
      stream.write( 2,"=\"" );
      stream.write(v.length(), v.c_str() );
      stream.write( 1,"\"" );
   }
	else
   {
...
void TiXmlComment::Print( Stream& stream, int depth ) const
{
	for ( int i=0; i<depth; i++ )
	{
      stream.write(3, "   " );
...
void TiXmlText::Print( Stream& stream, int /*depth*/ ) const
{
	TIXML_STRING buffer;
	PutString( value, &buffer );
   stream.write(buffer.length(), buffer.c_str() );
}
...
void TiXmlData::Print( FILE* cfile, int /*depth*/ ) const
{
	TIXML_STRING buffer;
	PutString( value, &buffer );
	fprintf( cfile, "<![CDATA[%s]]>", buffer.c_str() );
}


void TiXmlData::Print( Stream& stream, int /*depth*/ ) const
{
	TIXML_STRING buffer;
	PutString( value, &buffer );
   stream.write( 9, "<![CDATA[" );
   stream.write(buffer.length(), buffer.c_str() );
   stream.write( 3, "]]>" );
}


void TiXmlData::StreamOut( TIXML_OSTREAM * stream ) const
{
	(*stream) << "<![CDATA[";
	PutString( value, stream );
	(*stream) << "]]>";
...
TiXmlNode* TiXmlData::Clone() const
{	
	TiXmlData* clone = 0;
	clone = new TiXmlData( "" );
...
void TiXmlDeclaration::Print( Stream& stream, int /*depth*/ ) const
{
	stream.write(6, "<?xml ");
   
	if ( !version.empty() )
   {
		stream.write(9, "version=\"" );
      stream.write(version.length(), version.c_str ());
      stream.write( 2, "\" " ); 
   }

	if ( !encoding.empty() )
   {
      stream.write( 10,"encoding=\"" );
      stream.write( encoding.length(),encoding.c_str () );
      stream.write( 2,"\" " );
   }
      
   if ( !standalone.empty() )
   {
		stream.write(12, "standalone=\"");
      stream.write(standalone.length(), standalone.c_str ());
      stream.write(2, "\" " ); 
   }
...
void TiXmlUnknown::Print( Stream& stream, int depth ) const
{
	for ( int i=0; i<depth; i++ )
		stream.write( 3, "   " );
