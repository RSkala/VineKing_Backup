// NOTICE:  THIS IS A MODIFIED VERSION OF THE TINYXML HEADER!  IT HAS BEEN MODIFIED TO 
// _NOT_ INCLUDE WINDOWS.H
...
public:
	...
	*/
   virtual void Print( Stream& stream, int depth ) const = 0;
...
public:
	...
	*/
	...
	TiXmlData*	   ToData()    const		{ return ( this && type == DATA     ) ? (TiXmlData*)     this : 0; } ///< Cast to a more defined type. Will return null not of the requested type.
...
public:
   ...
   virtual void Print( Stream& stream, int depth ) const;
...
/** XML data. Contained in an element.
*/
class TiXmlData : public TiXmlNode
{
	friend class TiXmlElement;
public:
	/// Constructor.
	TiXmlData (const char * initValue) : TiXmlNode (TiXmlNode::DATA)
	{
		SetValue( initValue );
	}
	virtual ~TiXmlData() {}
	...
	#ifdef TIXML_USE_STL
	/// Constructor.
	TiXmlData( const std::string& initValue ) : TiXmlNode (TiXmlNode::DATA)
	{
		SetValue( initValue );
	}
	#endif
...
public:
	...
	*/
	bool LoadFile();
   bool LoadStream( FileStream& stream );
   bool SaveStream( FileStream& stream );
