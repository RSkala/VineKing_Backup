...
private:
   ...
   {
      Stream *stream;
      if(stream = zip->openFile(filename, ZipArchive::Write))
      {
   ...
   bool readWriteTest(const char *zipfile)
   {
      ...
      Stream *wrStream, *rdStream;
      if(wrStream = zip->openFile("writeTest.txt", ZipArchive::Write))
      {
         wrStream->writeLine((U8 *)"Test text to make a valid file");

         // This next open should fail
         if(rdStream = zip->openFile("writeTest.txt", ZipArchive::Read))
         {
