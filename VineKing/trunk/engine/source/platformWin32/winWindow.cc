...
// Center/size the window based on whether it has a menu or not.
void adjustWindowSize( bool hasMenu )
{
   // Adjust the window rect to compensate for the window style:
   RECT windowRect;

   GetClientRect( winState.appWindow, &windowRect );

   // Need to update platform window child rect?
   //AdjustWindowRect( &windowRect, GetWindowLong( winState.appWindow, GWL_STYLE ), hasMenu );
   ...
   if(adjWidth < winState.desktopClientWidth)
      xPos = ( winState.desktopClientWidth - adjWidth ) / 2;

   if(adjHeight < winState.desktopClientHeight)
      yPos = ( winState.desktopClientHeight - adjHeight ) / 2;

   SetWindowPos( winState.appWindow, 0, xPos, yPos, adjWidth, adjHeight, SWP_NOZORDER );
}
...
static void InitOpenGL()
{
   ...
   adjustWindowSize( Platform::HasMenuBar() );
}
...
S32 Platform::runBatchFile( const char* fileName, const char* batchArgs, bool locking )
{
	return -1;
}

bool Platform::isBatchFileDone( int fileID )
{
	return false;
}
...
//--------------------------------------
ConsoleFunction( isKoreanBuild, bool, 1, 1, "isKoreanBuild() Checks if current build is Korean" 
				"@return Returns true if Korean build, false otherwise")
{
   argc; argv;
   HKEY regKey;
   bool result = false;
