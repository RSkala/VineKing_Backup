...
bool OpenGLDevice::setScreenMode( U32 width, U32 height, U32 bpp, bool fullScreen, bool forceIt, bool repaint )
{
   ...
   if ( !newFullScreen && ( newRes.w >= winState.desktopClientWidth || newRes.h >= winState.desktopClientHeight ) )
   {
      ...
      for ( S32 i = 0; i < mResolutionList.size(); i++ )
      {
         ...
         else if( newRes.h <= winState.desktopClientHeight && newRes.w <= winState.desktopClientWidth )
         {
...
bool OpenGLDevice::setGammaCorrection(F32 g)
{
   ...
   for (U16 i = 0; i < 256; ++i)
      ramp[i] = (U16)mPow((F32) i/256.0f, g) * 65535.0f;
...
DisplayDevice* OpenGLDevice::create()
{
   ...
   if ( chosenFormat != 0 )
   {
      ...
      else if ( getOSVersion() == OS_NT && result && pfd.cColorBits == 16 )
         fullScreenOnly = true;
   }
