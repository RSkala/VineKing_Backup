...
#include "platformiPhone/SoundEngine.h"
#endif
...
// RKS: 2012/09/22 -- I COMMENTED OUT ALL THE LINES STARTING FROM HERE: - This shit was causing compile errors
//#ifdef TORQUE_OS_MAC
//static ALCdevice *mDevice   = NULL;             // active OpenAL device
//static ALCcontext *mContext = NULL;             // active OpenAL context
//#elif TORQUE_OS_IPHONE
static ALCdevice *mDevice   = NULL;             // active OpenAL device
static ALCcontext *mContext = NULL;             // active OpenAL context
//#else
...
static const F32 g_fDefaultAudioVolume = 0.9f; // RKS NOTE: I added this to default all audio volumes to this value. Values are also in defaultPrefs.cs.
...
// UNUSED: JOSEPH THOMAS -> static ALuint mEnvironment = 0;                        // al environment handle
   ...
   if (grp != NULL)
   {
      ...
      for (; itr != grp->end(); itr++)
      {
         ...
         if(profile != NULL && profile->isPreload())
         {
           ...
           // UNUSED: JOSEPH THOMAS ->  if((bool)buffer)
           // UNUSED: JOSEPH THOMAS ->     ALuint bufferId = buffer->getALBuffer();
         }
      }
   }
   return(true);
...
//#else
   ...
   for(U32 i = 0; i < Audio::NumAudioTypes; i++)
      ...
      mAudioTypeVolume[i] = g_fDefaultAudioVolume; // RKS NOTE: I changed this because audio was defaulting to 1.0f. I commented out the above line.
