...
void GuiIconButtonCtrl::inspectPostApply()
{
   ...
   if ((mBounds.extent.x == 0) && (mBounds.extent.y == 0) && mTextureNormal)
   {
      ...
      mBounds.extent.x = texture->bitmapWidth + 4;
      mBounds.extent.y = texture->bitmapHeight + 4;
   }
}
...
void GuiIconButtonCtrl::setBitmap(const char *name)
{
   ...
   if (*mBitmapName)
   {
      mTextureNormal = TextureHandle(name, BitmapTexture, true);
   }
   else
   {
      mTextureNormal = NULL;
...
void GuiIconButtonCtrl::renderButton( Point2I &offset, const RectI& updateRect )
{
   ...
   if ( mTextureNormal && mIconLocation != GuiIconButtonCtrl::IconLocNone )
   {
      ...
      {
         RectI iconRect(offset + mButtonMargin, Point2I(texture->bitmapWidth,texture->bitmapHeight));
         Point2I textureSize( texture->bitmapWidth, texture->bitmapHeight );
   ...
   if(mTextLocation != TextLocNone)
   {
      ...
      if(mTextLocation == TextLocRight)
      {
         ...
         {
            TextureObject *texture = (TextureObject *) mTextureNormal;
            start.x = texture->bitmapWidth + mButtonMargin.x + mTextMargin;
         }
      ...
      if(mTextLocation == TextLocCenter)
      {
         ...
         if( mTextureNormal && mIconLocation != GuiIconButtonCtrl::IconLocNone )
         {
            ...
            start.set( ( (mBounds.extent.x - textWidth - texObject->bitmapWidth)/2) + texObject->bitmapWidth, (mBounds.extent.y-mProfile->mFont->getHeight())/2 );
         }
         else
