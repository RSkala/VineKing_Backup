...
#if PUAP_OPTIMIZE
   //don't draw a blank rect for every control, every frame, just do one for the Canvas
#else
...
bool GuiControl::renderTooltip(Point2I cursorPos, const char* tipText )
{
   ...
   // Hardcoding this for now since we're using platform cursors and
   // getCursorExtent returns 0.
   offset.y += 22;//root->getCursorExtent().y;
   //Create text bounds.
...
bool GuiControl::pointInControl(const Point2I& parentCoordPoint)
{
   ...
   S32 yt = parentCoordPoint.y - mBounds.point.y;
	...
	// RKS: Testing to see why no touch up event occurs when dragging hand off side of screen
	//printf( "GuiControl::pointInControl: %s\n", getName() );
	//printf( " - mBounds.point.x:    %d\n", mBounds.point.x );
	//printf( " - mBounds.point.y:    %d\n", mBounds.point.y );
	//printf( " - mBounds.extent.x:   %d\n", mBounds.extent.x );
	//printf( " - mBounds.extent.y:   %d\n", mBounds.extent.y );
	//printf( " - parentCoordPoint.x: %d\n", parentCoordPoint.x );
	//printf( " - parentCoordPoint.y: %d\n", parentCoordPoint.y );
	//printf( " - xt:                 %d\n", xt );
	//printf( " - yt:                 %d\n", yt );
	
	bool bRet = xt >= 0 && yt >= 0 && xt < mBounds.extent.x && yt < mBounds.extent.y; // RKS:  For debugging
	if( bRet == false )
	{
		int breakhere = 0;
		breakhere++;
	...
	//return bRet;
	return true; // RKS: THIS IS JUST A TEST TO SEE IF DOING THIS FIXES THE TOUCH UP PROBLEM
...
GuiControl* GuiControl::findHitControl(const Point2I &pt, S32 initialLayer)
{
   ...
   while (i != begin())
   {
      ...
      else if (ctrl->mVisible && ctrl->pointInControl(pt)) // RKS: This is where the touch up fails! (inside the call to pointInControl())
      {
