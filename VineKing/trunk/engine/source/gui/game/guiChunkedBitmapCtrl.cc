...
ConsoleMethod( GuiChunkedBitmapCtrl, setBitmap, void, 3, 3, "(string filename)"
			  ...
			  "@param filename The location of the bitmap.\n"
...
void GuiChunkedBitmapCtrl::onRender(Point2I offset, const RectI &updateRect)
{
   if(mTexHandle)
   {
      if (mTile)
      {
         // UNUSED: JOSEPH THOMAS -> int stepy = 0;
         for(int stepy = 0; stepy < mBounds.extent.y; stepy += mTexHandle.getHeight())
         {
            // UNUSED: JOSEPH THOMAS -> int stepx = 0;
            for(int stepx = 0; stepx < mBounds.extent.x; stepx += mTexHandle.getWidth())
