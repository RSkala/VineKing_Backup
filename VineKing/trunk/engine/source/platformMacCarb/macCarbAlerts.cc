...
static S32 _MacCarbRunAlert( DialogRef alert )
{
   ...
   PlatStateMac::get().setHideCursor(false);
   ...
   PlatStateMac::get().restoreHideCursor();
...
{
   DialogRef alert = _MacCarbCreateAlert(windowTitle, message,CFSTR("Yes"),CFSTR("No"));
   S32 hit = _MacCarbRunAlert(alert);
   return ( hit == kAlertStdAlertOKButton );
}
