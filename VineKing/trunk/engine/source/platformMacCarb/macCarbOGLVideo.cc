...
bool OpenGLDevice::enumDisplayModes(CGDirectDisplayID display)
{
   ...
   for(int i=0; i < len; i++)
   {
      ...
      // get width
      num = (CFNumberRef) CFDictionaryGetValue( mode, kCGDisplayWidth );
      CFNumberGetValue(num, kCFNumberLongType,&width);
      
      // get height
      num = (CFNumberRef) CFDictionaryGetValue( mode, kCGDisplayHeight );
      CFNumberGetValue(num, kCFNumberLongType,&height);
      
      // get bpp
      num = (CFNumberRef) CFDictionaryGetValue( mode, kCGDisplayBitsPerPixel );
