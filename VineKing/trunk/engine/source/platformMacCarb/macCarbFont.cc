...
#pragma mark -
// Callback for enumeratePlatformFonts. Gets called once per found font.
static OSStatus _MacCarbEnumFontsCallback(ATSFontRef font, void* fontVector)
{
   if(!font)
      return paramErr;
   ...
   ATSFontGetName(font,kATSOptionFlagsDefault,&fontName);
   
   // convert the cfsting font name to utf8
   U32 cfsLen = CFStringGetLength(fontName);
   U32 bufLen = CFStringGetMaximumSizeForEncoding( cfsLen, kCFStringEncodingUTF8) + 1; // +1 for the null terminator
   FrameTemp<UTF8> buf(bufLen);
   CFStringGetCString( fontName, buf, bufLen, kCFStringEncodingUTF8);
   
   // put it in the name vector
   Vector<StringTableEntry>* fonts = (Vector<StringTableEntry>*)fontVector;
   fonts->push_back( StringTable->insert(buf));
...
#pragma mark -
#pragma mark  old font, to be removed in next version

// The old font is going away. Stubbing it out for now, but
//  leaving the full code commented out at the bottom of this file.
