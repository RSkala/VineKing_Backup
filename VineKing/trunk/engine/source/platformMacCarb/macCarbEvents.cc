...
EventHandlerRef gAppComEventHandlerRef    = NULL;

// Workaround for unexpected behavior in Apple's cocoa-carbon integration.
// The ClickActivationResult returned by the kEventWindowGetClickActivation event
// is not reliably respected. A fast click ignores the result, a slow click honors it.
// Refer also to comments in _OnMouseUpDown() for more information.
bool sgMacCarbHandledActivationClick;
...
static void _OnMouseUpDown(EventRef theEvent, bool down, U32 modifiers )
{
   if(down)
   {
      // Handles the edge case of click-activtion, updating the torque mouse coords.
      // 
      // The carbon click-activation event cannot be used, because the mouse
      // coords we get from that event are not reliable. They may be in
      // global or local space, and the event is sometimes delivered twice:
      // once by the cocoa-carbon integration object NSCarbonWindow, once by
      // the carbon event loop itself. There does not appear to be a way
      // to distinguish these two events at this time, nor to prevent the
      // double-delivery.
...
static OSStatus _OnWindowEvent(EventHandlerCallRef nextHandler, EventRef theEvent, void* userData)
{
   ...
   ClickActivationResult activClickResponse;
   ...
   switch(eventKind)
   {
      ...
      case kEventWindowGetClickActivation:
         // this is the activating click, activate torque and ignore it.
         //
         // we check for backgrounded here because this event is erroneously delivered
         // with every click, under 10.4
         if(platState.backgrounded)
         {
            sgMacCarbHandledActivationClick = true;
            activClickResponse = kActivateAndIgnoreClick;
            SetEventParameter(theEvent,kEventParamClickActivation,
                  typeClickActivationResult,sizeof(ClickActivationResult),&activClickResponse);
            return noErr;
         }
         break;
      default:
...
static OSStatus _OnAppMouseEvent(EventHandlerCallRef nextHandler, EventRef theEvent, void* userData)
{
   ...
   if(result==noErr || sgMacCarbHandledActivationClick)
   {
      sgMacCarbHandledActivationClick = false;
      return result;
   }
...
//-----------------------------------------------------------------------------
OSErr _OnOpenDocumentsAE(const AppleEvent *theAppleEvent, AppleEvent *reply, long handlerRefcon)
{
   AEDescList  docList;
   FSRef       theFSRef;
   long        index;
   ...
   // we can only open 1 document at once, all others are ignored.
   count = getMax(count, 1);
   
   if( count >= 1)
   {
     err = AEGetNthPtr(&docList, 1, typeFSRef, NULL, NULL, &theFSRef, sizeof(theFSRef), NULL);
     FSRefMakePath(&theFSRef, (UInt8*)path, 2048);
   }
   
   if(Con::isFunction("MainOpenProject"))
      Con::executef( 2, "MainOpenProject", path );
   else
      Con::errorf("Could not open project file because main.cs has not yet run.");
...
static OSStatus _OnCommandEvent(EventHandlerCallRef nextHandler, EventRef theEvent, void* userData)
{
   ...
   if(commandStruct.attributes & kHICommandFromMenu)
      if(MacCarbHandleMenuCommand(&commandStruct))
         result = noErr;
...
static OSStatus _OnTorqueEvent(EventHandlerCallRef nextHandler, EventRef theEvent, void* userData)
{
   ...
   switch( eventKind )
   {
      ...
      case kEventTorqueModalDialog:
         MacCarbShowDialog(torqueData);
         break;
         
      case kEventTorqueDrawMenuBar:
         DrawMenuBar();
         break;
   }
   return noErr;
...
void MacCarbInstallTorqueCarbonEventHandlers()
{
   ...
   if (torqueHandlerUPP==NULL)
   {
      ...
      torqueEventTypes[c].eventClass = kEventClassTorque;
      torqueEventTypes[c++].eventKind = kEventTorqueModalDialog;
      torqueEventTypes[c].eventClass = kEventClassTorque;
      torqueEventTypes[c++].eventKind = kEventTorqueDrawMenuBar;
...
void MacCarbInstallCarbonEventHandlers()
{
   ...
   static EventTypeSpec    comEventTypes[MAX_EVENT_TYPES];
   static EventHandlerUPP  comEventHandlerUPP = NULL;
   static int              comEventCount = 0;
   
   if(comEventHandlerUPP == NULL)
   {
      c = 0;
      comEventTypes[c].eventClass = kEventClassCommand;
      comEventTypes[c++].eventKind = kEventCommandProcess;
      
      comEventHandlerUPP = NewEventHandlerUPP(_OnCommandEvent);
      comEventCount = c;
   }
...
//      winEventTypes[c++].eventKind = kEventWindowDeactivated;
   ...
   InstallEventHandler(GetApplicationEventTarget(), comEventHandlerUPP, comEventCount,      comEventTypes,      NULL, &gAppComEventHandlerRef);
   ...
   AEInstallEventHandler(kCoreEventClass, kAEOpenDocuments, NewAEEventHandlerUPP(_OnOpenDocumentsAE), 0, false);}
...
void MacCarbRemoveCarbonEventHandlers()
{
   ...
   RemoveEventHandler(gAppComEventHandlerRef);
