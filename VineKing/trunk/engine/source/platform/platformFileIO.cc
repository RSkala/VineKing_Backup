...
StringTableEntry Platform::stripBasePath(const char *path)
{
   ...
   if(str = tryStripBasePath(path, Platform::getMainDotCsDir()))
      return str;

   if(str = tryStripBasePath(path, Platform::getCurrentDirectory()))
      return str;

   if(str = tryStripBasePath(path, Platform::getPrefsPath()))
      return str;
