...
bool Video::setDevice( const char *renderName, U32 width, U32 height, U32 bpp, bool fullScreen )
{
   ...
   //Luma: -svenfix - must remove all this code here to only care for GL
...
bool Video::setScreenMode( U32 width, U32 height, U32 bpp, bool fullScreen )
{
   if ( smCurrentDevice )
   {
      ...
      if(width == 0)
         width = IPHONE_DEFAULT_RESOLUTION_X;
      if(height == 0)
         height = IPHONE_DEFAULT_RESOLUTION_Y;
      if(bpp == 0)
