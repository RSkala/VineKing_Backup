...
#ifdef TORQUE_DEBUG
ConsoleFunction(ValidateMemory, void, 1, 1, "() Validates all memory blocks"
...
ConsoleFunction(FreeMemoryDump, void, 1, 1, "() Use the freeMemoryDump function to dump the current 'memory free' statistics to the console.\n"
...
{
   S32 tmp = countUnflaggedAllocs(file);
}

S32 countUnflaggedAllocs(const char * filename, S32 *outUnflaggedRealloc)
{
