...
//    output: RUNNING
//
// EVAL tag frame expr - evaluate the expr in the console, on the frame'th DictionaryStack frame
...
//          breakpoint.  It lists out one file/line/function triplet for each DictionaryStack level.
//          The first one is the top of the DictionaryStack.
//
...
void TelnetDebugger::pushStackFrame()
{
   ...
   if(mBreakOnNextStatement && mStackPopBreakIndex > -1 && 
      gEvalState.DictionaryStack.size() > mStackPopBreakIndex)
      setBreakOnNextStatement( false );
}
...
void TelnetDebugger::popStackFrame()
{
   ...
   if(mStackPopBreakIndex > -1 && gEvalState.DictionaryStack.size()-1 <= mStackPopBreakIndex)
      setBreakOnNextStatement( true );
}

void TelnetDebugger::breakProcess()
{
   // Send out a break with the full DictionaryStack.
   sendBreak();
...
void TelnetDebugger::sendBreak()
{
   ...
   for(S32 i = (S32) gEvalState.DictionaryStack.size() - 1; i >= last; i--)
   {
      CodeBlock *code = gEvalState.DictionaryStack[i]->code;
      const char *file = "<none>";
      if (code && code->name && code->name[0])
         file = code->name;

      Namespace *ns = gEvalState.DictionaryStack[i]->scopeNamespace;
      scope[0] = 0;
      if ( ns ) {
		  //-Mat more mPackage referencing
         if ( ns->getOwner() && ns->getOwner()->mName && ns->getOwner()->mName[0] ) {
            dStrcat( scope, ns->getOwner()->mName );
            dStrcat( scope, "::" );
         }
      ...
      const char *function = gEvalState.DictionaryStack[i]->scopeName;
      if ((!function) || (!function[0]))
         function = "<none>";
      dStrcat( scope, function );

      U32 line=0, inst;
      U32 ip = gEvalState.DictionaryStack[i]->ip;
      if (code)
...
void TelnetDebugger::debugStepOver()
{
   ...
   setBreakOnNextStatement( true );
   mStackPopBreakIndex = gEvalState.DictionaryStack.size();
   mProgramPaused = false;
   send("RUNNING\r\n");
}
...
void TelnetDebugger::debugStepOut()
{
   ...
   setBreakOnNextStatement( false );
   mStackPopBreakIndex = gEvalState.DictionaryStack.size() - 1;
...
void TelnetDebugger::evaluateExpression(const char *tag, S32 frame, const char *evalBuffer)
{
   ...
   if ( frame > gEvalState.DictionaryStack.size() )
      frame = gEvalState.DictionaryStack.size() - 1;
   if ( frame < 0 )
      frame = 0;
...
#ifdef PUAP_SCRIPT_CHANGE
   BufferStream stream;
   newCodeBlock->compileToStream( &stream, NULL, buffer );
   newCodeBlock->read( "TelDebuggerEval", stream );
   
   Namespace *currentNamespace = getCurrentNamespace();

   const char* result = newCodeBlock->exec(0, NULL, currentNamespace, 0, 0, false, NULL, frame);
#else
