...
void CodeBlock::addToCodeList()
{
   // remove any code blocks with my name
	//-Mat we at least want to warn here, and may want to disallow overwriting existing code
...
//-Mat check this one
   ...
   while(identCount--)
   {
      ...
      U32 count;
      st.read(&count);
		//-Mat this is where we connect a variable with it's string. this count is the number of times 
		//it's used, and ip is where, now that we have a STE, we copy it to each of these locations
		//normal(non-variable) strings don't do this
      while(count--)
      {
         U32 ip;
         st.read(&ip);
...
bool CodeBlock::compile(const char *codeFileName, StringTableEntry fileName, const char *script)
{
   ...
   bool result = compileToStream( &st, fileName, script );
   return result;
...
{
	//-Mat a bit of hackery with the BufferStream,but it sure beats re-writing 
	//all the write() functions to save to a buffer
...
const char *CodeBlock::compileExec(StringTableEntry fileName, const char *string, bool noCalls, int setFrame)
{
   ...
   name = fileName;
	...
	//<Mat> filename is only used to add the string into the string table, but name
	//is used when "registering" with the code list to remove anything with the same name
   if(fileName)
   {
      const StringTableEntry exePath = Platform::getMainDotCsDir();
      const StringTableEntry cwd = Platform::getCurrentDirectory();
   ...
   statementList = NULL;
	//-Mat use compileToBuffer to compileExec()
   //-Mat not working with TGB Builder at the moment
#if 0
//#ifdef PUAP_SCRIPT_CHANGE
	//here is the compile part of this function
   //char *codeBuffer = NULL;
   BufferStream streamBuffer;
   compileToBuffer( &streamBuffer, fileName, string );
	
   if( !read( fileName, streamBuffer ) ) {
	   return false;//WTF m8?
   }
#else
   ...
   if(lastIp != codeSize)
	...
	//here is the exec part (execs just like normal)
   //right namespace
