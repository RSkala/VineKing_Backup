...
//-Mat these are the sizes of the various nodes
#include "astNodeSizes.h"

#define MAT_DSO_NULL_VALUE (U32)(0x00000000)
#define MAT_NULL_VALUE	0x00000000
...
{
	//-Mat this IS throwing breakPairs off
   addBreakLine(ip);
...
U32 ReturnStmtNode::precompileStmt(U32)
{
   addBreakCount();
   if(!expr)
      return NODE_SIZE_RETURNSTMTNODE;
   else
      return NODE_SIZE_RETURNSTMTNODE + expr->precompile(TypeReqString);
}
...
U32 IfStmtNode::precompileStmt(U32 loopCount)
{
   ...
   if(!elseBlock)
      endifOffset = ifSize + NODE_SIZE_IFSTMTNODE + exprSize;
   else
   {
      elseOffset = exprSize + NODE_SIZE_IFSTMTNODE + ifSize + NODE_SIZE_IFSTMTNODE;
...
U32 LoopStmtNode::precompileStmt(U32 loopCount)
{
   ...
   if(!isDoLoop)
   {
      loopBlockStartOffset = initSize + testSize + NODE_SIZE_LOOPSTMTNODE;
      continueOffset       = loopBlockStartOffset + blockSize;
      breakOffset          = continueOffset + endLoopSize + testSize + NODE_SIZE_LOOPSTMTNODE;
   }
   else
   {
...
U32 ConditionalExprNode::precompile(TypeReq type)
{
   ...
   return exprSize + 
      ...
      falseExpr->precompile(type) + NODE_SIZE_CONDITIONALEXPRNODE;
}
...
U32 StreqExprNode::precompile(TypeReq type)
{
   ...
   // OP_COMPARE_STR
   // optional conversion
   U32 addSize = left->precompile(TypeReqString) + right->precompile(TypeReqString) + NODE_SIZE_STREQEXPRNODE;
   if(!eq)
      addSize ++;
   if(type != TypeReqUInt)
      addSize ++;
...
{
   U32 addSize = left->precompile(TypeReqString) + right->precompile(TypeReqString) + NODE_SIZE_STRCATEXPRNODE;
   if(appendChar)
      addSize++;
...
{
   U32 addSize = left->precompile(TypeReqString) + right->precompile(TypeReqString) + NODE_SIZE_COMMACATEXPRNODE;
   if(type != TypeReqString)
      addSize ++;
   return addSize;
}
...
U32 IntUnaryExprNode::precompile(TypeReq type)
{
   ...
   if(type != TypeReqUInt)
      return exprSize + NODE_SIZE_INTUNARYEXPRNODE_REQUINT;
   else
      return exprSize + NODE_SIZE_INTUNARYEXPRNODE;
}
...
U32 FloatUnaryExprNode::precompile(TypeReq type)
{
   ...
   if(type != TypeReqFloat)
      return exprSize + NODE_SIZE_FLOATUNARYEXPRNODE_REQUINT;
   else
      return exprSize + NODE_SIZE_FLOATUNARYEXPRNODE;
}
...
U32 VarNode::precompile(TypeReq type)
{
   ...
   if(arrayIndex)
      return arrayIndex->precompile(TypeReqString) + NODE_SIZE_VARNODE_REQSTRING;
   else
      return NODE_SIZE_VARNODE;
}
...
U32 VarNode::compile(U32 *codeStream, U32 ip, TypeReq type)
{
   ...
   //-Mat add a blank(for now) pointer before each string when it gets read
   codeStream[ip] = MAT_DSO_NULL_VALUE;
   ip++;
...
{
   if(type == TypeReqNone)
      return NODE_SIZE_FLOATNODE_REQNONE;
   if(type == TypeReqString)
      index = getCurrentStringTable()->addFloatString(value);
   else if(type == TypeReqFloat)
      index = getCurrentFloatTable()->add(value);
   return NODE_SIZE_FLOATNODE;
}
...
U32 StrConstNode::precompile(TypeReq type)
{
   ...
   if( doc )
   {
      ...
      return NODE_SIZE_STRCONSTNODE_DOC;
   }

   if(type == TypeReqString)
   {
      index = getCurrentStringTable()->add(str, true, tag);
      return NODE_SIZE_STRCONSTNODE_REQSTRING;
   }
   else if(type == TypeReqNone)
   {
      return NODE_SIZE_STRCONSTNODE_REQNONE;
   }

   fVal = consoleStringToNumber(str, dbgFileName, dbgLineNumber);
   if(type == TypeReqFloat)
      index = getCurrentFloatTable()->add(fVal);
   return NODE_SIZE_STRCONSTNODE;
}
...
U32 ConstantNode::precompile(TypeReq type)
{
   if(type == TypeReqString)
   {
      ...
      return NODE_SIZE_CONSTANTNODE_REQSTRING;
   }
   else if(type == TypeReqNone)
      return NODE_SIZE_CONSTANTNODE_REQNONE;

   fVal = consoleStringToNumber(value, dbgFileName, dbgLineNumber);
   if(type == TypeReqFloat)
      index = getCurrentFloatTable()->add(fVal);
   return NODE_SIZE_CONSTANTNODE;
}
...
U32 AssignExprNode::precompile(TypeReq type)
{
   ...
   if(arrayIndex)
   {
      if(subType == TypeReqString)
         return arrayIndex->precompile(TypeReqString) + retSize + addSize + NODE_SIZE_ASSIGNEXPRNODE_ARRARYINDEX_REQSTRING;
      else
         return arrayIndex->precompile(TypeReqString) + retSize + addSize + NODE_SIZE_ASSIGNEXPRNODE_ARRARYINDEX;
   }
   else
      return retSize + addSize + NODE_SIZE_ASSIGNEXPRNODE;
}
...
U32 AssignOpExprNode::precompile(TypeReq type)
{
   ...
   if(type != subType)
      size++;
   if(!arrayIndex)
      return size + NODE_SIZE_ASSIGNOPEXPRNODE_ARRAYINDEX;
   else
   {
      size += arrayIndex->precompile(TypeReqString);
      return size + NODE_SIZE_ASSIGNOPEXPRNODE;
   }
}
...
U32 FuncCallExprNode::precompile(TypeReq type)
{
   ...
   return size + NODE_SIZE_FUNCCALLEXPRNODE;
}
...
U32 SlotAccessNode::precompile(TypeReq type)
{
   ...
   if(arrayExpr)
   {
      ...
      // OP_TERMINATE_REWIND_STR
      // OP_SETCURFIELDARRAY
      // total add of 3 + array precomp
      size += NODE_SIZE_SLOTACCESSNODE_REQSTRING_ARRAYEXPR + arrayExpr->precompile(TypeReqString);
   }
   // eval object expression sub + 3 (op_setCurField + OP_SETCUROBJECT)
   size += objectExpr->precompile(TypeReqString) + NODE_SIZE_SLOTACCESSNODE_REQSTRING;

   // get field in desired type:
   return size + NODE_SIZE_SLOTACCESSNODE;
}
...
U32 InternalSlotAccessNode::precompile(TypeReq type)
{
   ...
   U32 size = NODE_SIZE_INTERNALSLOTACCESSNODE;
   
   // eval object expression sub + NODE_SIZE_INTERNALSLOTACCESSNODE + (op_setCurField + OP_SETCUROBJECT)
...
U32 SlotAssignNode::precompile(TypeReq type)
{
   ...
   if(objectExpr)
      size += objectExpr->precompile(TypeReqString) + NODE_SIZE_SLOTASSIGNNODE_REQSTRING;
   else
      size += NODE_SIZE_SLOTASSIGNNODE_REQSTRING;

   if(arrayExpr)
      size += arrayExpr->precompile(TypeReqString) + NODE_SIZE_SLOTASSIGNNODE_ARRAYEXPR;
   return size + NODE_SIZE_SLOTASSIGNNODE;
}
...
U32 SlotAssignOpNode::precompile(TypeReq type)
{
   ...
   if(type != subType)
      size++;
   if(arrayExpr)
      return size + NODE_SIZE_SLOTASSIGNOPNODE_ARRAYEXPR + arrayExpr->precompile(TypeReqString) + objectExpr->precompile(TypeReqString);
   else
      return size + NODE_SIZE_SLOTASSIGNOPNODE + objectExpr->precompile(TypeReqString);
}
...
U32 ObjectDeclNode::precompileSubObject(bool)
{
   ...
   U32 nameSize = objectNameExpr->precompile(TypeReqString) + NODE_SIZE_OOBJECTDECLNODE_SUBOBJECT_NAME;
   ...
   failOffset = NODE_SIZE_OOBJECTDECLNODE_SUBOBJECT + nameSize + argSize + slotSize + subObjSize;
   return failOffset;
}
...
U32 ObjectDeclNode::precompile(TypeReq type)
{
   ...
   U32 ret = NODE_SIZE_OBJECTDECLNODE + precompileSubObject(true);
   if(type != TypeReqUInt)
      return ret + 1;
   return ret;
}
...
U32 FunctionDeclStmtNode::precompileStmt(U32)
{
   ...
   endOffset = argc + subSize + NODE_SIZE_FUNCTIONDECLSTMTNODE + ( argc * NODE_SIZE_FUNCTIONDECLSTMTNODE_PER_ARG);
   return endOffset;
}
...
U32 FunctionDeclStmtNode::compileStmt(U32 *codeStream, U32 ip, U32, U32)
{
   ...
   #ifdef TORQUE_EXTRA_BREAKLINES      
   //-Mat this may be throwing Breakpairs off
      addBreakLine(ip);   
   #endif
