																...
																"@return Returns a copy of tokenList, less the first token and the first delimiter. If there are no more tokens, a NULL string is returned.")
{
   ...
   if (str)
   {
      ...
      if (gEvalState.DictionaryStack.size() && 
         gEvalState.DictionaryStack.last()->scopeName)
																...
																"@sa error, warn")
{
   ...
   Con::printf("%s", ret);
	//printf( "CONSOLE: %s\n", ret ); // RKS:  I need some sort of debug output, so I added this (08/08/2010 Note: This doesn't seem necessary any more, as we get normal output.)
   ret[0] = 0;
}
...
#ifndef TORQUE_OS_IPHONE //-Mat no batch files on iPhone (yet?)
//----------------------------------------------------------------
//	CHRIS:	functions for executing batch files from script
ConsoleFunction( runBatchFile, const char*, 3, 4, "( fileName,  args , locking) - Creates a process that executes a batch file.\n" 
																"@param fileName - A string containing the name of a batch file to be executed.\n"
																"@param args - Any arguments that should be passed to the batch file.\n"
																"@param locking - if true will cause this function to wait till batch file ends.\n"
																"	false will cause the function to exit as soon as the process is started.\n"
																"@return Returns a string containing the id of the batch file that is executed.")
{
	argc;

	S32 batchID = Platform::runBatchFile(argv[1], argv[2], dAtob(argv[3]));

	char* idBuff = Con::getReturnBuffer(16);
   
	dSprintf(idBuff,16, "%d", batchID);

	return idBuff;
}

ConsoleFunction( isBatchFileDone, bool, 2, 2, "( batchID ) - returns true when batch file is done executing." )
{
	argc;
	S32 batchID = dAtoi(argv[1]);

	return Platform::isBatchFileDone(batchID);
}
																...
																"@sa exec")
{
   ...
   // UNUSED: JOSEPH THOMAS -> Stream *compiledStream = NULL;
   ...
   char scriptFullNameBuffer[1024];
   Platform::makeFullPathName( scriptFilenameBuffer, scriptFullNameBuffer, sizeof(scriptFullNameBuffer) );
   dStrcpy( scriptFilenameBuffer, scriptFullNameBuffer );
...
ConsoleFunction(compilePath, const char*, 2, 2, "compilePath( path )")
{
	...
	if ( !Con::expandScriptFilename(expandedFilename, sizeof(expandedFilename), argv[1]) )
		return "-1 0";
	...
	while ( match = ResourceManager->findMatch( expandedFilename, &compileArgs[1], match ) )
	{
																...
																"@sa compile")
{
   ...
   StringTableEntry scriptFileName = StringTable->insert("");
...
//release tests on iPhone is irrelevant.
   ...
   if(!ResourceManager->find(scriptFilenameBuffer))
   {
      ...
      if(!ResourceManager->find(nameBuffer))
         scriptFileName = Platform::getPrefsPath(Platform::stripBasePath(scriptFilenameBuffer));
      else
         scriptFileName = StringTable->insert(scriptFilenameBuffer);
   }
   else
      scriptFileName = StringTable->insert(scriptFilenameBuffer);
#else
...
//Also, no DSO generation on iPhone
   ...
   if(compiledStream)
   {
      ...
      F32 st1 = Platform::getRealMilliseconds();
      ...
      code->exec(0, scriptFileName, NULL, 0, NULL, noCalls, NULL, 0);
	  	...
	  	F32 et1 = Platform::getRealMilliseconds();
   ...
   else if(rScr) //Luma : Load normal cs file here.
   {
      ...
      //Luma: Profile script executions 
	   ...
	   //if( dStrcmp( scriptFileName, "/private/var/mobile/Applications/05F342EE-E0BF-4776-B75F-F97D5567F053/VineKingDebug.app/game/data/levels/Level_TestLevel01.t2d" ) == 0 ) // RKS: To check when the level is loaded
	   //{
		//   int shit = 0;
		//   shit++;
	   //}
   ...
   {
      // Don't have anything.
      Con::warnf(ConsoleLogEntry::Script, "Missing file: %s!", scriptFilenameBuffer);
      ret = false;
   }
				...
				"@return Returns a boolean value which is true if the given mathod name represents a valid method in the namespace and false otherwise.")
{
   ...
   //if we can't get a namespace, it's probably an object being passed to us, so find it and use it's Namespace
   if( ns == NULL ) {
		SimObject *pObj = Sim::findObject( argv[1]);
		ns = pObj->getNamespace();
		//still no namespace so we're boned
		if( ns == NULL ) {
			return false;
		}
   }
																...
																"@return No return value")
{
   ...
   {
#ifdef TORQUE_PLAYER
      Con::expandScriptFilename(scriptFilenameBuffer, sizeof(scriptFilenameBuffer), argv[2]);
...
//Luma:	Console function to tell if this is a TORQUE_OS_IPHONE build
