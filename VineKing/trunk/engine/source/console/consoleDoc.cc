...
{
	//-Mat implement printNamespaceEntries
	/*	
   static bool inGroup = false;

   // Go through all the entries.
   // Iterate through the methods of the namespace...
   U32 size = g->mChildEntryList->getSize();
   Entry * currentEntry = NULL;
   for(U32 i = 0; i < size ; i++ )
   {
	   currentEntry = g->mChildEntryList->getElement( i );
      char buffer[1024]; //< This will bite you in the butt someday.
      int eType = currentEntry->mType;
      const char * funcName = currentEntry->mFunctionName;
      ...
      if(eType >= Entry::ScriptFunctionType || eType == Entry::OverloadMarker)
      {
         if(eType==Entry::OverloadMarker) 
         {
            ...
            // Find the original
            eType = 8;
			Entry *seekEntry = NULL;
            for(U32 j = 0; j < g->mChildEntryList->getSize(); j++ )
            {
				seekEntry = g->mChildEntryList->getElement( j );
               if(!dStrcmp(seekEntry->mFunctionName, currentEntry->cb.mGroupName))
               {
                  eType = seekEntry->mType;
                  break;
               }
            }
            // And correct the name
            funcName = currentEntry->cb.mGroupName;
         }
         ...
         const char *use = currentEntry->mUsage ? currentEntry->mUsage : "";
         ...
         // Default...
         printClassMethod(true, typeNames[eType], funcName, "", currentEntry->mUsage);
      }
      else if(currentEntry->mType == Entry::GroupMarker)
      {
         if(!inGroup)
            printGroupStart(currentEntry->cb.mGroupName, currentEntry->mUsage);
         else 
            printGroupEnd();

         inGroup = !inGroup;
      }
      else if(currentEntry->mFunctionOffset)                 // If it's a builtin function...
      {
         currentEntry->mCode->getFunctionArgs(buffer, currentEntry->mFunctionOffset);
         printClassMethod(false, typeNames[currentEntry->mType], currentEntry->mFunctionName, buffer, "");
      }
      else
      {
         Con::printf("   // got an unknown thing?? %d", currentEntry->mType );
      }
   }
   */
}
...
{
	//-Mat implement hashTable for dumpClasses this needs top be completely re-done
#if 0
...
{
	//-Mat implement, this seems to think that the Package holding a namespace will have Entrys too
/*	
   // Get the global namespace.
	Namespace* g = Package::getMainNamespace(); //->mParent;
