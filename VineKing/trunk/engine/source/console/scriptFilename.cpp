...
bool expandGameScriptFilename(char *filename, U32 size, const char *src)
{
   ...
   // we must have a game expando, otherwise we can't expand it
   if (!game)
   {
      *filename = 0;
      return false;
   ...
   if (dStrncmp(src, "~/", 2) == 0)
   {
      ...
      // now convert to full path
      Platform::makeFullPathName(ptr, filename, size, game->mPath);
      return true;
   }
   else if ((dStrncmp(src, "./", 2) == 0) || (src[0] == '/'))
   {
      Con::errorf("Illegal path specified: %s", src);
      *filename = 0;
      return false;
   }
   else
   {
      ...
      const char* slash = dStrrchr(game->mPath, '/');

      // we add one here because we want the trailing /
      U32 length = (slash - game->mPath) + 1;

      dStrncpy(filename, game->mPath, length);
      dStrcpy(filename + length, src);
...
#ifdef TORQUE_TOOLS
ConsoleFunction(expandGameScriptFilename, const char*, 2, 2, "(string filename)")
{
   argc;
   char* ret = Con::getReturnBuffer( 1024 );
   Con::expandGameScriptFilename(ret, 1024, argv[1]);
   return ret;
}
#endif
