...
enum TextureHandleType
{
   ...
   ZeroBorderTexture,         ///< Clears the border of the texture on all mip levels
	...
	ImageMap2DTexture,				///< OFF LIMITS - for image-map 2D use only.  You WILL cause problems if you use this type.
	ImageMap2DKeepTexture			///< OFF LIMITS - for image-map 2D use only.  You WILL cause problems if you use this type.
};
...
struct TextureManager
{
  ...
  public:
   ...
   static GBitmap *loadBitmapInstance(const char *textureName, bool recurse = true, bool nocompression = false);
